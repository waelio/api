...
io.on('connection', (socket) => {
    ...
    socket.on('client:event', (data: unknown) => {
    ...
    socket.on('chat:message', (payload: ChatPayload, ack?: (response: { ok: boolean; error?: string }) => void) => {
        try {
            const normalized = {
                id: socket.id,
            ...
            if (typeof ack === 'function') ack({ ok: false, error: (err as Error)?.message || 'error' })
        }
    })
