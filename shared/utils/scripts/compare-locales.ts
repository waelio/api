...
function flatten(obj: NestedObject, prefix = ''): Record<string, boolean> {
    const res: Record<string, boolean> = {}
    for (const k of Object.keys(obj)) {
        const val = obj[k]
        const key = prefix ? `${prefix}.${k}` : k
        if (val && typeof val === 'object' && !Array.isArray(val)) {
            Object.assign(res, flatten(val as NestedObject, key))
        }
        else {
            res[key] = true
        }
...
const en = JSON.parse(fs.readFileSync(path.join(baseDir, 'en.json'), 'utf8')) as NestedObject
const ar = JSON.parse(fs.readFileSync(path.join(baseDir, 'ar.json'), 'utf8')) as NestedObject
const de = JSON.parse(fs.readFileSync(path.join(baseDir, 'de.json'), 'utf8')) as NestedObject
